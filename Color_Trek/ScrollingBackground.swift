...
//        Initialized Background Image
        let bgImage = UIImage(named: image)!
        
//        Initialized Scroll Node
    ...
    func update (currentTime:TimeInterval) {
        for child in self.children {
            ...
            if child.position.x <= -child.frame.size.width {
                ...
                child.position = CGPoint(x: child.frame.size.width * CGFloat(CGFloat(self.children.count - 1) + delta), y: child.position.y)
            }
        }
    }
